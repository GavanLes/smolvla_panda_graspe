...
class SimpleEnv2:
    ...
    def render(self, teleop=False, idx=0, clean_ui=True):
        """
        clean_ui=True: 只显示主仿真画面，不显示任何四周信息/小窗/调试标记
        """

        if clean_ui:
            # 关键：不画任何 overlay / debug
            self.env.render()
            return
        ...
        if getattr(self, 'instruction', None) is not None:
            self.env.viewer_text_overlay(text1='Language Instructions', text2=self.instruction)
